defmodule Exflect.Detect do
  ...
  def singular?(word), do: Enum.any?(singular_patterns(), &Regex.match?(&1, word)) || !plural?(word)
  def plural?(word), do: Enum.any?(plural_patterns(), &Regex.match?(&1, word))
end
